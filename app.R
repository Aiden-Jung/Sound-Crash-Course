...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel(
      ...
      tags$br(),
      tags$ol(
        tags$li("Arandjelovi??, R. and Zisserman, A., 2018. Objects that Sound. [online] Deepmind. Available at: <https://deepmind.com/blog/article/objects-that-sound> [Accessed 8 December 2021]."),
        tags$li("Corey, R., 2021. How do face masks affect speech? | Innovation in Augmented Listening Technology - University of Illinois at Urbana-Champaign. [online] Publish.illinois.edu. Available at: <https://publish.illinois.edu/augmentedlistening/face-mask-acoustics/> [Accessed 8 December 2021]."),
        tags$li("Shahid, F., 2021. Google Nows Let You 'Hum to Search' the Song Stuck in Your Head. [online] Wccftech. Available at: <https://wccftech.com/google-nows-let-you-hum-to-search-the-song-stuck-in-your-head/> [Accessed 8 December 2021]."),
        tags$li("BYJUS. 2021. What Is Timbre? - Theory & Characteristics. [online] Available at: <https://byjus.com/physics/timbre/> [Accessed 8 December 2021].")
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$file, {
    suff <- str_sub(input$file$name, -4, -1)
    if (!(suff == ".mp3" || suff == ".wav")) {
      output$warning <- renderText("Invalid file format")
    } else {
  ...
  observeEvent(input$files, {
    suff <- str_sub(input$files, -4, -1)
    if (!(suff == ".mp3" || suff == ".wav")) {
    } else {
      if (suff == ".mp3") {
        values$oldWave <- readMP3(paste0("www/set1/", input$files, sep = ""))
      } else {
        values$oldWave <- readWave(paste0("www/set1/", input$files, sep = ""))
      }
      values$newWave <- values$oldWave
    }
